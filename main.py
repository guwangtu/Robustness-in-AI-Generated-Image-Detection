...
def prepare_dataloader(data_paths,train_transform,val_transform,combine_all=True,args=None):
    ...
    for i in range(len(train_data)):
        ...
        if val_rate<1.0:
            val_data[i].shrink_dataset(args.val_rate)
...
def run_get_imgs(args,trainer,model):
    ...
    if not args.val_only:
        trainer.save_imgs(model,train_loader[0],save_path+'/train', args.adv)
    trainer.save_imgs(model,val_loader[0],save_path+'/test', args.adv)
